...
═══════════════════════════════════════════════════════ */
        ...
        async function parseJSONSafe(response) {
            ...
            try {
                ...
                return { error: text.trim() || 'Server returned invalid JSON.' };
            }
        }
        ...
        function renderNextStep() {
            ...
            updateWorkflowStatusCards();
        }

        function getNextExamSummary() {
            var exams = APP_DATA.studentTools.exams || [];
            if (!exams.length) return 'No exams scheduled';
            var sorted = exams.slice().filter(function(item) {
                return item && item.date;
            }).sort(function(a, b) {
                return new Date(a.date) - new Date(b.date);
            });
            if (!sorted.length) return 'No exams scheduled';
            var next = sorted[0];
            var dateLabel = new Date(next.date).toLocaleDateString('en-IN', { month: 'short', day: 'numeric' });
            return next.name + ' • ' + dateLabel;
        }

        function getStudyPercentText() {
            var total = getTrackerTotal();
            var target = APP_DATA.studentTools.timeTracker.targetMinutes || 0;
            if (!target) return 'Set a weekly target';
            var pct = Math.min(100, Math.round((total / target) * 100));
            return pct + '% of target';
        }

        function updateWorkflowStatusCards() {
            var goal = document.getElementById('status-goal-focus');
            var study = document.getElementById('status-study-time');
            var studyPct = document.getElementById('status-study-percent');
            var accuracy = document.getElementById('status-accuracy-value');
            var exam = document.getElementById('status-next-exam');
            if (goal) {
                goal.textContent = APP_DATA.studentTools.nextStep || 'No next step pinned yet.';
            }
            if (study) {
                study.textContent = getTrackerTotal() + ' min logged';
            }
            if (studyPct) {
                studyPct.textContent = getStudyPercentText();
            }
            if (accuracy) {
                var totalGoals = APP_DATA.studentTools.items.achieve.length + APP_DATA.studentTools.items.achieved.length;
                var pct = totalGoals ? Math.round((APP_DATA.studentTools.items.achieved.length / totalGoals) * 100) : 0;
                accuracy.textContent = pct + '%';
            }
            if (exam) {
                exam.textContent = getNextExamSummary();
            }
        }
        ...
        async function validateSessionWithServer() {
            ...
            // Frontend-only mode: trust local session data instead of backend validation.
            updateAuthNav();
            return true;
        }
        ...
        async function doSignup() {
            ...
            try {
                // Backend signup disabled for frontend-only mode.
                // Bypass /api/v1/auth/signup and create a local session.
                var data = {
                    accessToken: 'local-signup-token',
                    user: {
                        name: name,
                        email: email,
                        role: APP_DATA.userData.role || 'Student',
                        emailVerified: true
                    }
                };
                ...
                closeMod();
                unlockSite();
                showToast('✅', 'Account created and signed in (frontend-only mode).');
            } catch (err) {
        ...
        async function doForgotPassword() {
            ...
            try {
                // Backend forgot-password disabled for frontend-only mode.
                sessionStorage.setItem('resetEmail', email);
                alert('Password reset is simulated locally. Use the reset form to continue.');
        ...
        async function doResetPassword() {
            ...
            try {
                // Backend reset-password disabled for frontend-only mode.
                alert('Password reset simulated locally. You can now login with your email.');
                openLoginPage();
            } catch (err) {
                console.error(err);
                alert('Connection error.');
        ...
        async function doLogin() {
            ...
            try {
                // Backend login disabled for frontend-only mode.
                // This comment-out bypasses /api/v1/auth/login and allows the page to work locally.
                /*
                ...
                */
                ...
                closeMod();
                unlockSite();
                showToast('✅', 'Signed in successfully (frontend-only mode).');
            } catch (err) {
                showToast('❌', err.message);
        ...
        async function verifyOTP() {
            ...
            try {
                // Backend OTP verification disabled for frontend-only mode.
                ...
                unlockSite();
                showToast('✅', 'Email verified successfully (frontend-only mode)!');
            } catch (err) {
                showToast('❌', err.message);
        ...
        async function resendOTP() {
            try {
                // Backend OTP resend disabled for frontend-only mode.
        ...
        async function initGoogleAnalytics() {
            // Backend config fetch disabled for frontend-only mode.
            // No analytics config will be loaded when running locally without /api/v1/config.
        }
